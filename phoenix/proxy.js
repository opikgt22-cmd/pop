...
async function scrapeFreeProxyCz() {
    const url = 'https://free-proxy.cz/en/proxylist/country/all/all/ping/all';
    try {
        ...
        $('table#proxy_list tbody tr').each((i, el) => {
            const ipAddress = $(el).find('td:nth-child(1)').text().trim();
            const port = $(el).find('td:nth-child(2)').text().trim();

            // Dekripsi port (Free-proxy.cz mengenkripsi port dengan Javascript)
            const portDecrypted = decryptPort($, el);

            if (ipAddress && portDecrypted) {
                const proxy = `${ipAddress}:${portDecrypted}`;
                proxies.push(proxy);
            }
        });
        ...
        console.error('Error scraping free-proxy.cz:', error);
        return [];
    }
}

// Fungsi untuk mendekripsi port
function decryptPort($, el) {
    let port = '';
    $(el).find('td:nth-child(2) span.fport').each((i, span) => {
...
async function scrapeProxies() {
    ...
    // Tambahkan free-proxy.cz ke daftar proxy
    const freeProxyCzProxies = await scrapeFreeProxyCz();
    proxies = proxies.concat(freeProxyCzProxies);
    ...
    for (const source of proxySources) {
        try {
            ...
            console.log(`Error Scraping ${source}`);
        }
    }
...
// Jalankan fungsi scrapeProxies (pastikan ini dijalankan hanya jika file dijalankan langsung)
